      ...
      <section className="flex items-center justify-center px-6 py-10">
        <form onSubmit={onSubmit} className="w-full max-w-md rounded-md border border-slate-200 bg-white p-8 shadow-xl dark:border-slate-800 dark:bg-slate-950">
          ...
          <label className="mt-8 block text-sm font-medium text-slate-700 dark:text-slate-300">
            {t("name")}
            <input value={name} onChange={(event) => setName(event.target.value)} className="mt-2 h-11 w-full rounded-md border border-slate-200 px-3 text-slate-950 outline-none focus:border-slate-500 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-50" />
          </label>
          ...
          <label className="mt-5 block text-sm font-medium text-slate-700 dark:text-slate-300">
            {t("role")}
            <select value={role} onChange={(event) => setRole(event.target.value as Role)} className="mt-2 h-11 w-full rounded-md border border-slate-200 px-3 text-slate-950 outline-none focus:border-slate-500 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-50">
              {roles.map((item) => (
