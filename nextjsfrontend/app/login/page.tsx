...
export default function LoginPage() {
  ...
  return (
    <main className="grid min-h-screen bg-slate-50 text-slate-950 lg:grid-cols-[1fr_480px]">
      <section className="hidden content-center bg-slate-950 px-12 text-white lg:grid">
        <div className="max-w-xl">
          ...
          <h1 className="text-5xl font-semibold leading-tight">Your store operations desk is ready.</h1>
          <p className="mt-5 text-lg leading-8 text-slate-300">
            Review orders, adjust stock, and keep branch teams aligned.
          </p>
        </div>
      </section>
      ...
      <section className="flex items-center justify-center px-6 py-10">
        <form onSubmit={onSubmit} className="w-full max-w-md rounded-md border border-slate-200 bg-white p-8 shadow-xl">
          ...
          <h2 className="mt-8 text-3xl font-semibold">Login</h2>
          ...
          <p className="mt-5 text-sm text-slate-500">
            Need an account? <Link href="/register" className="font-semibold text-slate-950">Register</Link>
          </p>
        </form>
      </section>
    </main>
